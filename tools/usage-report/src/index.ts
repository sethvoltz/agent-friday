...
function printStats(label: string, stats: PeriodStats): void {
  ...
  const avgCost =
    stats.turns > 0 ? formatCost(stats.cost / stats.turns) : "—";
  console.log(
    `  ${label}: ${formatCost(stats.cost)} across ${stats.turns} turns (avg ${avgCost}/turn)`
  );
}
...
function main(): void {
  ...
  const sessionMap = new Map<
    string,
    { type: string; turns: number; cost: number }
  >();
  for (const e of entries) {
  ...
  const orchestratorSessions = [...sessionMap.values()].filter(
    (s) => s.type === "orchestrator"
  );
  const independentSessions = [...sessionMap.values()].filter(
    (s) => s.type === "independent"
  );
  ...
  console.log();
  console.log(
    `  Orchestrator: ${orchTurns} turns, ${formatCost(orchCost)} (${orchestratorSessions.length} sessions)`
  );
  console.log(
    `  Independent:  ${indTurns} turns, ${formatCost(indCost)} (${independentSessions.length} sessions)`
  );
  ...
  if (verbose) {
    ...
    console.log(
      `  Cache creation: ${allStats.cacheCreationTokens.toLocaleString()}`
    );
    console.log(
      `  Cache read:     ${allStats.cacheReadTokens.toLocaleString()}`
    );
  }
