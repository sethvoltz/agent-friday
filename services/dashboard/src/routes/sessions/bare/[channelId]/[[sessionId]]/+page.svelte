<script lang="ts">
  ...
  // Bare sessions use channelId as the streaming key (matches eventAgentName in client.ts)
  const streamText = $derived(getStreamingText(data.channelId));
...
<style>
  .transcript-page { max-width: 900px; display: flex; flex-direction: column; height: 100%; }
  ...
  .turn-list-wrapper { position: relative; flex: 1; min-height: 0; }
  .turn-list { display: flex; flex-direction: column; gap: 1.5rem; height: 100%; overflow-y: auto; }
  .scroll-btn {
    position: absolute; right: 1rem; width: 2rem; height: 2rem;
    border-radius: 50%; border: 1px solid var(--border-primary);
    background: var(--bg-card); color: var(--text-secondary); font-size: 0.9rem;
    cursor: pointer; display: flex; align-items: center; justify-content: center;
    box-shadow: var(--shadow-sm); transition: all var(--transition-fast); z-index: 5;
  }
  .scroll-btn:hover { background: var(--bg-tertiary); color: var(--text-primary); }
