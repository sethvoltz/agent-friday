...
import { listProposals, updateProposal, type Proposal } from "./store.js";

const CLUSTERS_DIR = join(EVOLVE_DIR, "clusters");
...
function filePath(id: string): string {
  return join(CLUSTERS_DIR, `${id}.md`);
}
...
export function listClusters(): Cluster[] {
  ensureClustersDir();
  const files = readdirSync(CLUSTERS_DIR).filter((f) => f.endsWith(".md"));
  const out: Cluster[] = [];
  for (const file of files) {
    const id = basename(file, ".md");
    try {
      out.push(parseCluster(id, readFileSync(join(CLUSTERS_DIR, file), "utf-8")));
    } catch {
      // Skip malformed.
    }
  }
...
export interface MergeOptions {
  /** Jaccard overlap on signal-hash sets above which two proposals merge into a cluster. Default 0.5. */
  threshold?: number;
}
...
/**
 * Cluster open/critical proposals by Jaccard overlap of their signal-hash sets.
 *
 * Strategy:
 *   1. For each pair of unclustered proposals, compute Jaccard. If ≥ threshold,
 *      they belong together.
 *   2. Use union-find to collapse the resulting graph into components.
 *   3. For each component with ≥ 2 members, create or update a cluster file
 *      and stamp `clusterId` on every member proposal.
 *
 * Non-destructive: proposal ids are preserved; only `clusterId` changes.
 * Re-runnable: existing clusters get their member list refreshed if the
 * graph still includes the same anchor.
 */
export function mergeClusters(opts: MergeOptions = {}): MergeResult {
  ...
  // Union-find over proposal indices.
...
function clusterTitleFor(members: Proposal[]): string {
  // Highest-scoring member's title becomes the cluster title — stable across
  // re-runs unless the ranking changes.
