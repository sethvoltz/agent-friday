...
- **Beads stays as the local agent scratchpad.** When the orchestrator claims a Linear ticket, it creates a *lightweight Beads epic shim* with a 1–2 line summary description and `linear_ticket=FRI-XX` metadata. Sub-task decomposition happens locally in beads — these never sync back to Linear (high-frequency, ephemeral, doesn't need to traverse the network).
- **Linkage is bidirectional.** Bead carries `linear_ticket` metadata; Linear ticket gets a back-reference comment with the marker `🔗 Friday bead: \`<bead-id>\`` (the third-party MCP doesn't expose attachments, so a structured comment substitutes — daemon-side reconciliation searches comments for the marker).
